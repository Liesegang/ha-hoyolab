...
from .const import (
    ...
    SENSOR_HI3_BOUNTY, SENSOR_HI3_COCKOON_WEEKLY,
)
...
GENSHIN_SENSORS: list[HoyoSensorDescription] = [
    ...
    HoyoSensorDescription(
        ...
        attr_fn=lambda d: {"max": d.get("max_expedition_num", 5)},
    ),
    HoyoSensorDescription(
...
ZZZ_SENSORS: list[HoyoSensorDescription] = [
    ...
    HoyoSensorDescription(
        ...
        attr_fn=lambda d: {"max": d.get("bounty_commission", {}).get("total", 4)},
    ),
    HoyoSensorDescription(
        ...
        icon="mdi:magnify",
        state_class=SensorStateClass.MEASUREMENT,
        value_fn=lambda d: d.get("survey_points", {}).get("num", 0),
        attr_fn=lambda d: {
            "max": d.get("survey_points", {}).get("total", 5000),
            "is_max_level": d.get("survey_points", {}).get("is_max_level", False),
        },
    ),
]
